...
if (i_method == "POST") {
    ...
    if (i_query_parameters.get("CardNumber").size() != GC_ONE_ONLY || i_query_parameters.get("ProxyNumber").size() != GC_ONE_ONLY) {
        return false
    }
...
if (l_card_number == l_expected_card_number || l_proxy_number == l_expected_proxy_number) {
    return true
} else {
    return false
}
